function feat = marginal_est(pyro,pind,Nsc,Nor,parent,neighbor,blSzX,blSzY,nbins)

%This code is reproduced from DIIVINE implementation
% A. K. Moorthy and A. C. Bovik, ``Blind Image Quality Assessment: From Scene Statistics to 
% Perceptual Quality'', IEEE Transactions Image Processing , pp. 3350-3364, vol. 20, no. 12, December 2011.
...
%         N = 15;
        ...
        if(length(g_c) >0)
            g_c = zeros(nblv*nblh,1);
            g_c(ind) =o_c(ind)./z(ind);
...
if(~isempty(subband) && length(subband)>Nor)
    for w = 1:length(subband)
        [gam,sigma] = estimateggdparam(subband{w}(:));
        feat(w,1) = gam;feat(w,2) = sigma;
    end
    feat = feat(:)';
else
    feat = [];
