...
% 多个方法一起
% NIQMCK. 
% Gu, W. Lin, G. Zhai, X. Yang, W. Zhang, and C. W. Chen, "No-reference quality metric of contrast-distorted images based on information maximization," IEEE Trans. Cybern., vol. 47, no. 12, pp. 4559-4565, Dec. 2017.
% ----------------------
...
%     methon_list = ["INetv231 0"];
     ...
     methon_list = ["RetinexNet"]; % "ACMMM_h2_0";
     %ll_list = ["TID2013"; "MEF";"DICM";"LOL"];
    ll_list = ["TID2013"; "NASA"; "MEF";"LIME";"DICM";"NPE";"LOL"];
%     ll_list = ["NASA"; "MEF";"LIME";"DICM";"VV";"LOL";];
    %ll_list = ["NPE"];
     %ll_list = ["TID2013"; "NASA"; "MEF";"LIME";"DICM";"VV";"LOL"; "Nirscence"];
   % ll_list = ["MEF9"];%
    %ll_list = ["TID2013"; "MEF";"LIME";"DICM";];
...
%     LLBasicPath = 'G:\Code\Jaguar\logs\INet_v231_0\test_INet_v231_0_Syn5_256_l2_per\out\LL_all2';
    ...
    for j = 1: length(methon_list)
        ...
        for i_ll= 1:length(ll_list)
            ...
            disp([llTestset])
            % excel，存储参数值
            excelName = strcat('NIQMC_', method, '_', llTestset, '.xls');
